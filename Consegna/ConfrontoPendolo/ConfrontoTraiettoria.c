...
int main(int argv, char *argc[]){
    ...
    char *gnuplot_commands[300] = {
        ...
        "set output 'ConfrontoTraiettoria.png'",
        "end" 
    };
    ...
    fprintf(gnu, "set label 'x0=%.2lf' at screen 0.35, 0.17\n", atof(argc[1]));
    fprintf(gnu, "set label 'v0=%.2lf' at screen 0.35, 0.145\n", atof(argc[2]));
    fprintf(gnu, "set label 'w=%.2lf' at screen 0.35, 0.12\n", atof(argc[3]));
    fprintf(gnu, "set label 'dt=%.2lf' at screen 0.35, 0.10\n", atof(argc[5]));
    fprintf(gnu, "plot ");
    for (int i=0; i<4; i++){
